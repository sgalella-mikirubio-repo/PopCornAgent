    ...
    if len(tokens)>1:
        for i in range(1,len(tagged)):
            ...
            if tagged[i][0] == 'movie' or tagged[i][0] == 'film':
                if tagged[i-1][1]=='JJ': # Look if there is an adjective before
                ...
                if film_frame.undefined1 == []:
                    if tagged[i+1][1] == 'NNP':
                        if tagged[i][0] == 'United' and tagged[i+1][0] == 'Kingdom':
                            film_frame.country = 'UK'
                        else:
                            film_frame.undefined1.append(tagged[i][0])
                            film_frame.undefined1.append(tagged[i+1][0])
                            film_frame.undefined1 = film_frame.undefined1[0] + ' ' + film_frame.undefined1[1]
